...
- Install Arch (or derivitive, also works/tested on CachyOS) using the =Archinstall= script.  For audio, select =Pipewire=, for networking select =iwd=.
...
* Still left to do
  - [ ] Fix emacs saving temp files everywhere.
  - [ ] Look into Tailscale!
