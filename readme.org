...
* Still left to do
  - [ ] Fix Hyprland gestures
  - [ ] Switch back to Networkmanager front-end, but iwd backend.
  - [ ] Get everything from the taskbar into rofi, see if it can be replaced, or maybe Walker?  Try nwg-drawer again, as its the only one that searches the Documents directory
  ...
  - [ ] Look into Tailscale!
  - [X] Once and for fucking all, un-fuck =wnotify='s colors (replaced with xfce4-notifyd, Christ but CSS needs to die).
