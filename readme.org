...
- Install Arch (or derivitive, also works/tested on CachyOS) using the =Archinstall= script.  Choose =NetworkManager= and =Pipewire=.
...
* Still left to do
  - [ ] Break out laptop specific stuff.
...
* Notes on final system
** AppArmour
Really want to use this, really hate the configuration setup.  Also the god-awful memory overhead, but then that's a running theme with most applications these days.
** Break up into basic desktop setup and system hardening.
Should have one script for cloning dot files and installed supporting packages (i.e. desktop setup), and another for configuring/hardening the OS.
** Themeing
Hard-coded themes are boring, going out and searching for a theme via random pics is fun.  That's it :-)
** OpenSnitch
*** Motivation
*** Research
Is it fine-grained enough so that allowing curl to access =archlinux.org= doesn't mean the curl can then access anything?  If curl is called from another process, can it still access the same site?  Ideally not, and I should see a full chain, so that if a trojan uses it to download a compromised package, I'll be alerted.
Does it have a TTY client, or does this just not make sense given it's application-level, and not server-level?
