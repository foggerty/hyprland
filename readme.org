...
* Still left to do
  - [ ] Copy musi back ointo laptop (again).
  - [ ] Add Gateway to whatami IP.
  - [ ] Add check in paper script for when pywal16 craps itself on a previously working image; which seems to happen every second update.
    - [ ] Bonus: fix the fucking OOB errors that it keeps throwing up and see if I can get a PR approved.  Also, fuck Python.
  - [ ] Update whatami to enumerate mounted block devices.
  - [ ] Setup hibernation in bootstrap script.
  ...
  - [X] Look into Tailscale!
  - [X] Add pacman masking to bootstrap script.
  - [X] Go through Lobster's article on Omarchy's problems, make sure install script avoids them.
  - [X] Break out laptop specific stuff.
