Note: remove FireWire, VirtualBox, AMD, Microsoft, Surface, Macintosh, infiniband, ISDN, apple, ibm, raspberry, thrustmaster, toshiba, xfs, xen, eprom, lenovo, asus HP

cpuFreq Govener,


* Installation
- Install Arch (or derivitive, tested on CachyOS) using the ~archinstall~ script - for audio, select ~Pipewire~, for networking select ~NetworkManger~.
