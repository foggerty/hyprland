...
* TODO
  ...
  - [X] Powersaving stuff (tlp)
  - [ ] Use pyprland for monocle mode?
  - [ ] Use ~yad~ for wallpaper dialog?
  - [ ] Finish theming swaync
  - [ ] Mime types.  Use `xdg-mime default thunar.desktop inode/directory` etc so no need to include mimetypes in repo.
  - [ ] AppArmour
