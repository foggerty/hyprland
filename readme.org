...
* Still left to do
  - [ ] Go through =cachyos-settings= and understand what's ging on before applynig them.
  - [ ] Do the same with =cachyos-sysctl-manager=.
  ...
  - [ ] Look into Tailscale!
  - [X] See if zswap will work with a swapfile.
  - [X] Setup swapfile and zswap.
